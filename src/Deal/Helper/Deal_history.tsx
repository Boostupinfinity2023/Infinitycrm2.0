...
const  {deal_id} = useParams();
  ...
  useEffect(() => {
    ...
    token.then((JwtToken) => {
      fetch(GETDATA + '?action=view-history-deal-comment&auth=true&view=true&edit=false', {
        method: "POST",
        headers: {
        ...
        body: JSON.stringify({
          deal_id:deal_id,
          PAGE_REQUEST: 'GET_FORM_COMMENT_HISTORY_DEAL'
        })
      }).then((response) => {
  ...
  useEffect(() => {
    ...
    token.then((JwtToken) => {
      fetch(TASKAPI + '?action=view-task-data&auth=true&view=true&edit=false', {
        method: "POST",
        headers: {
          Authenticate: `Bearer ${JwtToken}`,
        },
        body: JSON.stringify({
            deal_id: deal_id,
          PAGE_REQUEST: 'GET_TASK_HISTROY_DATA_DEAL'
        })
      }).then((response) => {
        return response.json();
      }).then((data) => {
  ...
  useEffect(() => {
    ...
    token.then((JwtToken) => {
      fetch(GETDATA + '?action=get-deal-history&auth=true&view=true&edit=false', {
        method: "POST",
        headers: {
          Authenticate: `Bearer ${JwtToken}`,
        },
        body: JSON.stringify({
          deal_id:deal_id,
          PAGE_REQUEST: 'GET_DEAL_PAGE_HISTORY'
        })
      }).then((response) => {
        return response.json();
      }).then((data) => {
            ...
            <div key={value.ID}>
              <Card className="max-w-[100%] mt-2 border-radius-none bg-slate-100">
                <CardHeader className="flex gap-3" style={{ justifyContent: "space-between" }}>
                  ...
                  -  <span className="font-thin">{formatDate(value.CREATE_AT)}</span>
                  </span>
                  <Tooltip content={`CREATE BY `+value.CREATE_BY_USER_NAME}>
                    <Link to="#">
                      <Avatar showFallback src={value.CREATE_BY_USER_PROFILE} size="sm" />
                    </Link>
                  </Tooltip>
                </CardHeader>
                <Divider />
