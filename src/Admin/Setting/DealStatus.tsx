...
import { TableColumnsType, Menu, message, Tooltip } from 'antd';
...
import EditModal from './EditDeal';
import AddNewStatus from './AddDealStatus';
...
const App: React.FC = () => {
    ...
    const GetStudentData = async () => {
        ...
        try {
            const body = JSON.stringify({
                PAGE_REQUEST: 'GET_DEAL_STATUS_DATA_ADMIN_SIDE',
            });
    ...
    const DeleteData = async (event: any) => {
        ...
        if (result.isConfirmed) {
            const body = JSON.stringify({
                PAGE_REQUEST: 'GET_DEAL_STATUS_DATA_DELETE',
                DATA: event,
            });
    ...
    const columns: TableColumnsType = [
        ...
        //     width: 100,
        //     render: (event) => (
        //         <Chip data-column="Editable" variant="flat" key={event} className={`text-${event == 'false' ? 'danger' : 'success'} Editable5`}>
        //             {event}
        //         </Chip>
        //     ),
        // },
        {
            ...
            render: (event) => (
                <>
                    <span data-column="Action">
                        <div className="flex text-2xl gap-3 mobtabalign">
                            ...
                            <Tooltip showArrow={true} title="Delete status">
                ...
                //         <span className='action_btn_lead5 w-6' data-column="Action" >
                //             <svg data-column="Action" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-6">
                //                 <path
                //                     strokeLinecap="round"
                ...
                //             }}
                //         >
                //             <span className='action_btn_lead w-6' data-column="Action" >
    ...
    return (
        <>
            ...
            <div className={`table-background `} ref={tableRef}>
                <div className="m-3 flex justify-end">
                    <AddNewStatus />
                </div>
                <Table className="my-class-table table table_phone_width" columns={columns} dataSource={status} loading={loading}
                    locale={{
                        emptyText: (
                            <Empty
                                image={
