...
const Scrumboard = ({ setLoader }: { setLoader: (value: boolean | ((prev: boolean) => boolean)) => void }) => {
    ...
    useEffect(() => {
        ...
        fetch(GetUserAPI + `?view=karban.deal&${url}`, {
            ...
            body: JSON.stringify({
                PAGE_REQUEST: 'karban.data.deal.view.agent',
            }),
        })
            .then((res) => {
                return res.json();
    ...
    useEffect(() => {
        if (DragItem && OldDragItem && onDragNew && ItmeId && updateStatus === true) {
            MySwal.fire({
                title: 'Add Comment',
                input: 'textarea',
                ...
                if (result.isConfirmed && result.value) {
                    fetch(GetUserAPI + '?edit=karban.Deal.dragItem', {
                        ...
                        body: JSON.stringify({
                            PAGE_REQUEST: 'karban.data.update.deal.staff',
                            dealId: ItmeId,
     ...
     */
    const HandleClassChnage = () => {
        console.log('enter any type...')
    }
                                                ...
                                                <Grid>
                                                    <ul>
                                                        <li className="mt-2 ml-2">
                                                            <NavLink to={`/staff/client/view/${task.ID}/${task.CLIENT_ID}/${JSON.parse(task.LEAD_INFORMATION).CLIENT_IDS}/staff`}>
                                                    ...
                                                    <ul className="mb-2 mt-5 move-icon">
                                                        ...
                                                        <li className="mt-2 ml-2 flex">
                                                            ...
                                                            <li
                                                                className={`ml-2 rounded-xl pl-1 pr-1 text-white  ${task.COUNTRY_EXIT === 'primary' ? 'bg-danger' : task.COUNTRY_EXIT === 'secondary' ? 'bg-primary' : 'bg-danger'
                                                                    } `}
                                                            >
